...
<xsl:template name="summary">
	...
	<table align="center" class="details" border="0" cellpadding="5" cellspacing="2" width="95%">
		...
		<tr valign="top">
			...
			<!--<xsl:variable name="allCount" select="count(/testResults/*[attribute::s='*'])" />-->
			<xsl:variable name="allCount" select="$allFailureCount + $allSuccessCount" />
...
<xsl:template name="pagelist">
	...
	<table align="center" class="details" border="0" cellpadding="5" cellspacing="2" width="95%">
		...
		<xsl:for-each select="/testResults/*[not(@ts = preceding::*/@ts)]">
			<xsl:variable name="label" select="@lb" />
			<xsl:variable name="count" select="count(../*[@lb = current()/@lb])" />
			<xsl:variable name="failureCount" select="count(../*[@lb = current()/@lb][attribute::s='false'])" />
			<!--<xsl:variable name="testStatus" select="@s" />-->
			...
			<tr valign="top">
				...
				<td>
				<xsl:if test="@s='false'">
				  <a class="Failure"><xsl:attribute name="href">SubPage/<xsl:value-of select="@ts" />.html</xsl:attribute>
				  <xsl:value-of select="$label" />
					 <!--  <xsl:value-of select="@ts" />-->
				  </a>
				</xsl:if>
				<xsl:if test="@s='true'">
					<a><xsl:attribute name="href">SubPage/<xsl:value-of select="@ts" />.html</xsl:attribute>
					<xsl:value-of select="$label" />
					</a>
				  <!--<xsl:value-of select="$label" />-->
				</xsl:if>
