...
public class Helper {
    ...
    public static void createDir(String dirLocation) {
        try {
            ...
            Log.error("Error while creating result directory. Exception:" + e.getMessage());
        }
    }
    ...
    public static void loadResources(String resultFolder) throws IOException {
        try {
            ...
            Log.error("Error while copying appropriate resources. Exception:" + e.getMessage());
        }
    }
    ...
    public static void generateJmeterProperyFile(String resourceFolderPath) {
        try {
            ...
            HashMap<String, String> globalConfig = ConfigReader.generalProperties().getConfig();//new ConfigReader(resourceFolderPath + "Configuration/General.properties").getConfig();
            HashMap<String, String> jmxModuleMapping = ConfigReader.jmxMappingProperties().getConfig();//new ConfigReader(resourceFolderPath + "Configuration/JMX_Mapping.properties").getConfig();
            String[] modules = globalConfig.get("Modules").split(","); //System.getProperty("Modules").split(",");
            if (modules.length > 0) {
                ...
                Log.error("No modules present to run. Please check the run time parameters.");
            }
        } catch (Exception e) {
            Log.error("Error while setting up JmeterTestExecution.properties file. Exception:" + e.getMessage());
        }
    }
    ...
    public static void writeToFile(HashMap<String, String> jmxExec, String fileName) throws Exception {
        ...
        try {
            ...
            while (itr.hasNext()) {
                ...
                String value = jmxExec.get(key) == null ? "" : jmxExec.get(key);
                properties.setProperty(key, value);
            }
            properties.store(new FileOutputStream(fileName), "JmeterTestExecution.properties");
        } catch (Exception ex) {
            Log.error("Error while generating JmeterTestExecution.properties file. Exception:" + ex.getMessage());
        }
    }
    ...
    public static void archiveReports(String targetDirPath, String zipFileName) {
        try {
            ...
            Log.info("Archived report(s) earlier then " + days + " days.");
        } catch (Exception e) {
            Log.error("Error while archiving report(s). Exception:" + e.getMessage());
        }
    }
    ...
    public static void zipFiles(String zipperFileName, ArrayList<File> reportList) {
        try {
            ...
            zout.close();
            fout.close();
            Log.info("Archived report(s) with zipper name:" + zipperFileName);
        } catch (Exception e) {
            Log.error("Error while archiving report(s). Exception:" + e.getMessage());
        }
    }
